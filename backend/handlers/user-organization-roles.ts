...
const validRoles = z.enum(['owner', 'admin', 'member', 'guest'])
...
export default async function handler(
  ...
  try {
    ...
    if (orgDetailsError || !orgDetails) {
      logger.error(
        `[UserOrgRoles] Erro ao buscar detalhes da organização ${organization_id}: ${orgDetailsError?.message || 'Organização não encontrada.'}`,
      )
    ...
    if (orgDetails.is_personal && req.method !== 'GET') {
      logger.warn(
        `[UserOrgRoles] Tentativa de modificar organização pessoal ${organization_id} por user ${user_id}.`,
      )
      return handleErrorResponse(
        res,
        403,
        'Não é possível gerenciar membros de uma organização pessoal.',
      )
    }

    const requestingUserRole = await getUserRoleInOrganization(user_id, organization_id, token)
    ...
    if (req.method === 'GET') {
      ...
      logger.info(`[UserOrgRoles] Buscando membros para organization_id: ${organization_id}`)
    ...
    if (req.method === 'POST') {
      logger.info(`[UserOrgRoles] Adicionando novo membro à organization_id: ${organization_id}`)
      ...
      if (targetError || !targetMember) {
        return handleErrorResponse(
          res,
          404,
          'Membro não encontrado ou não pertence a esta organização.',
        )
      }
      ...
      if (
        ...
        return handleErrorResponse(
          res,
          403,
          "Admins não podem atribuir o papel de 'owner' a si mesmos.",
        )
      }
      ...
      if (targetMember.user_id === user_id) {
        ...
        if (
          ...
          return handleErrorResponse(
            res,
            403,
            "Você não pode se remover se for o último 'owner' ou 'admin' da organização.",
          )
        }
      }

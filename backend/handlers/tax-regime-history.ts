...
export default async function handler(
  ...
  try {
    ...
    if (!userOrgAndPeriod) {
      logger.warn(`[TaxRegimeHistory] Organização ou período contábil não encontrado para user_id: ${user_id}`);
    ...
    if (req.method === "POST") {
      ...
      if (!parsedBody.success) {
        logger.warn(`[TaxRegimeHistory] Erro de validação do corpo da requisição: ${parsedBody.error.errors.map((err) => err.message).join(", ")}`);
        return handleErrorResponse(
          res,
          400,
      ...
      logger.info(`[TaxRegimeHistory] Inserindo novo regime tributário para organization_id: ${organization_id}`);
      ...
      if (dbError) {
        logger.error(`[TaxRegimeHistory] Erro ao inserir novo regime tributário: ${dbError.message}`);
        throw dbError;
      }
      logger.info(`[TaxRegimeHistory] Novo regime tributário criado com sucesso: ${data.id}`);
      return res.status(201).json(data);
    } else if (req.method === "GET") {
      logger.info(`[TaxRegimeHistory] Buscando regimes tributários para organization_id: ${organization_id}`);
      const { data, error: dbError } = await userSupabase
        .from("tax_regime_history")
      ...
      logger.info(`[TaxRegimeHistory] Atualizando regime tributário ${id} para organization_id: ${organization_id}`);
      const parsedBody = updateTaxRegimeHistorySchema.safeParse(req.body);
      if (!parsedBody.success) {
        logger.warn(`[TaxRegimeHistory] Erro de validação do corpo da requisição PUT: ${parsedBody.error.errors.map((err) => err.message).join(", ")}`);
        return handleErrorResponse(
          res,
          400,
          parsedBody.error.errors.map((err) => err.message).join(", "),
      ...
      if (updateData.start_date || updateData.end_date) {
        ...
        if (fetchCurrentError || !currentRegime) {
          logger.error(`[TaxRegimeHistory] Erro ao buscar regime atual ${id}: ${fetchCurrentError?.message || "Regime não encontrado."}`);
        ...
        if (fetchError) {
          logger.error(`[TaxRegimeHistory] Erro ao buscar regimes existentes para PUT: ${fetchError.message}`);
          throw fetchError;
        }
      ...
      if (dbError) {
        logger.error(`[TaxRegimeHistory] Erro ao atualizar regime tributário ${id}: ${dbError.message}`);
        throw dbError;
      }
      if (!data) {
        logger.warn(`[TaxRegimeHistory] Regime tributário ${id} não encontrado ou sem permissão para atualizar.`);
        return handleErrorResponse(
          res,
          404,
      ...
      logger.info(`[TaxRegimeHistory] Deletando regime tributário ${id} para organization_id: ${organization_id}`);
      ...
      if (dbError) {
        logger.error(`[TaxRegimeHistory] Erro ao deletar regime tributário ${id}: ${dbError.message}`);
        throw dbError;
      }
      if (count === 0) {
        logger.warn(`[TaxRegimeHistory] Regime tributário ${id} não encontrado ou sem permissão para deletar.`);
        return handleErrorResponse(
          res,
          404,

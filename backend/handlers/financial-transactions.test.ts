...
describe("financialTransactionsHandler", () => {
  ...
  it("should return accounts payable for GET requests", async () => {
    ...
    vi.mocked(supabaseClient.mockSelect).mockResolvedValueOnce({
      data: mockData,
      error: null,
    });
    ...
    expect(vi.mocked(supabaseClient.mockFrom)).toHaveBeenCalledWith(
      "accounts_payable",
    );
  ...
  it("should create a new accounts payable for POST requests", async () => {
    req = {
      method: "POST",
      query: { type: "payable" },
      body: { amount: 150, description: "New Payable" },
    };
    const mockData = {
      id: "new-ap",
      amount: 150,
    ...
    vi.mocked(supabaseClient.mockSingle).mockResolvedValueOnce({
      data: mockData,
      error: null,
    });
    ...
    expect(vi.mocked(supabaseClient.mockInsert)).toHaveBeenCalledWith([
      expect.objectContaining({ amount: 150, description: "New Payable" }),
    ]);
  ...
  it("should create a new accounts receivable for POST requests", async () => {
    req = {
      ...
      body: { amount: 250, description: "New Receivable" },
    };
    const mockData = {
      id: "new-ar",
      amount: 250,
    ...
    expect(vi.mocked(supabaseClient.mockInsert)).toHaveBeenCalledWith([
      expect.objectContaining({ amount: 250, description: "New Receivable" }),
    ]);
    expect(res.status).toHaveBeenCalledWith(201);
    expect(res.json).toHaveBeenCalledWith(mockData);
  });
  ...
  it("should return 400 for invalid POST body", async () => {
    ...
    vi.mocked(
      schemas.createFinancialTransactionSchema.safeParse,
    ).mockReturnValueOnce({
      success: false,
      error: { errors: [{ message: "Invalid data" }] },
    });
    ...
    expect(vi.mocked(supabaseClient.handleErrorResponse)).toHaveBeenCalledWith(
      res,
      400,
      expect.any(String),
    );
  ...
  it("should update an existing financial transaction for PUT requests", async () => {
    req = {
      method: "PUT",
      query: { type: "payable", id: "trans-123" },
      body: { amount: 160, description: "Updated Payable" },
    };
    const mockData = {
      id: "trans-123",
      amount: 160,
      description: "Updated Payable",
      user_id,
    };
    vi.mocked(supabaseClient.mockEq).mockReturnValueOnce({
      select: vi.fn().mockResolvedValueOnce({ data: [mockData], error: null }),
    });
    ...
    expect(vi.mocked(supabaseClient.mockUpdate)).toHaveBeenCalledWith({
      amount: 160,
      description: "Updated Payable",
    });
    expect(vi.mocked(supabaseClient.mockEq)).toHaveBeenCalledWith(
      "id",
      "trans-123",
    );
    expect(res.status).toHaveBeenCalledWith(200);
  ...
  it("should return 404 if transaction not found for PUT", async () => {
    req = {
      method: "PUT",
      query: { type: "payable", id: "trans-not-found" },
      body: { amount: 160 },
    };
    vi.mocked(supabaseClient.mockEq).mockReturnValueOnce({
    ...
    expect(vi.mocked(supabaseClient.handleErrorResponse)).toHaveBeenCalledWith(
      res,
      404,
      "Transação não encontrada ou você não tem permissão para atualizar esta transação.",
    );
  });
  ...
  it("should return 404 if transaction not found for DELETE", async () => {
    ...
    expect(vi.mocked(supabaseClient.handleErrorResponse)).toHaveBeenCalledWith(
      res,
      404,
      "Transação não encontrada ou você não tem permissão para deletar esta transação.",
    );
  });
});

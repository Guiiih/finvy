...
const createAccountingPeriodSchema = z.object({
  name: z.string().min(1, "Nome do período é obrigatório.").max(100, "Nome do período muito longo."),
  start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Formato de data de início inválido. Use YYYY-MM-DD."),
  end_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Formato de data de fim inválido. Use YYYY-MM-DD."),
  is_active: z.boolean().optional().default(false),
});

const updateAccountingPeriodSchema = z.object({
  name: z.string().min(1, "Nome do período é obrigatório.").max(100, "Nome do período muito longo.").optional(),
  start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Formato de data de início inválido. Use YYYY-MM-DD.").optional(),
  end_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Formato de data de fim inválido. Use YYYY-MM-DD.").optional(),
...
export default async function handler(
  ...
  try {
    if (req.method === "GET") {
      // Retorna todos os períodos contábeis para a organização do usuário
      ...
      // Se o novo período for definido como ativo, desativa os outros períodos ativos da organização
      ...
      // Se o período estiver sendo definido como ativo, desativa os outros períodos ativos da organização

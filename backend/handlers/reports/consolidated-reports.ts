  ...
  try {
    ...
    if (format === "xlsx") {
      ...
      for (const month of sortedMonths) {
        ...
        doc.moveDown();
        doc.fontSize(10).text('Data        Descrição                                Conta                          Débito         Crédito');
        doc.text('-----------------------------------------------------------------------------------------------------');
        monthlyData[month].forEach((entry) => {
          entry.entry_lines.forEach((line) => {
            const accountName = accountMap.get(line.account_id) || line.account_id;
            doc.fontSize(8).text(`${entry.entry_date.padEnd(12)}${entry.description.padEnd(40)}${accountName.padEnd(30)}${String(line.debit).padEnd(15)}${String(line.credit).padEnd(15)}`);
          });
        });
        doc.moveDown();
      }

...
describe('generateReportsHandler', () => {
  ...
  it('should generate reports successfully with start and end dates', async () => {
    req = { method: 'GET', query: { startDate: '2024-01-01', endDate: '2024-12-31' } };
  ...
  it('should generate reports successfully without dates', async () => {
    ...
    expect(schemas.reportQuerySchema.safeParse).toHaveBeenCalledWith(req.query);
    expect(reportService.generateReports).toHaveBeenCalledWith(user_id, token, undefined, undefined);
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockReports);
  });
  ...
  it('should return 400 for invalid query parameters', async () => {
    ...
    vi.mocked(schemas.reportQuerySchema.safeParse).mockReturnValueOnce({ success: false, error: { errors: [{ message: 'Invalid date format' }] } });

    await generateReportsHandler(req, res, user_id, token);

    expect(supabaseClient.handleErrorResponse).toHaveBeenCalledWith(res, 400, 'Invalid date format');
  });

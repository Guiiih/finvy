...
vi.mock("../utils/schemas", () => ({
  yearEndClosingSchema: {
    safeParse: vi.fn((data: { closingDate: string }) => {
      ...
      return {
        success: false,
        error: { errors: [{ message: "Invalid date" }] },
      };
    }),
  },
}));
...
describe("yearEndClosingHandler", () => {
  ...
  it("should process year-end closing successfully", async () => {
    ...
    supabaseClient.mockLte.mockResolvedValueOnce({
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        message: expect.stringContaining(
          "Fechamento de exercício para 2024-12-31 realizado com sucesso.",
        ),
      }),
    );
  });
  ...
  it("should handle unexpected errors", async () => {
    ...
    vi.spyOn(schemas.yearEndClosingSchema, "safeParse").mockImplementation(
      () => {

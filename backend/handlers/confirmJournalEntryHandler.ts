  ...
  try {
    if (req.method === "POST") {
      ...
      if (!parsedBody.success) {
        logger.warn(
          `[ConfirmJournalEntryHandler] Erro de validação do corpo da requisição: ${
      ...
      logger.info(`[ConfirmJournalEntryHandler] ${createdEntries.length} lançamentos confirmados e criados.`);

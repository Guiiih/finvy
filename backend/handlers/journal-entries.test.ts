...
describe("journalEntriesHandler", () => {
  ...
  it("should return journal entries for GET requests", async () => {
    ...
    const mockData = [
      { id: "je1", entry_date: "2024-01-01", description: "Entry 1", user_id },
    ];
    supabaseClient.mockOrder.mockResolvedValueOnce({
      data: mockData,
      error: null,
    });
  ...
  it("should create a new journal entry for POST requests", async () => {
    req = {
      method: "POST",
      body: { entry_date: "2024-01-01", description: "New Entry" },
    };
    const mockData = {
      id: "new-je",
  ...
  it("should update an existing journal entry for PUT requests", async () => {
    req = {
      method: "PUT",
      query: { id: "je1" },
      body: { description: "Updated Entry" },
    };
    const mockData = {
      id: "je1",
      entry_date: "2024-01-01",
    ...
    supabaseClient.mockSelect.mockResolvedValueOnce({
      data: [mockData],
      error: null,
    });
  ...
  it("should handle unexpected errors", async () => {
    ...
    expect(handleErrorResponse).toHaveBeenCalledWith(
      res,
      500,
      "Erro inesperado na API de lançamentos.",
    );
  });
});

...
describe("profileHandler", () => {
  ...
  it("should return user profile for GET requests", async () => {
    ...
    supabaseClient.mockSingle.mockResolvedValueOnce({
      data: mockProfile,
      error: null,
    });
  ...
  it("should delete user for DELETE requests", async () => {
    req = { method: "DELETE" };
    supabaseClient.mockAuthAdmin.deleteUser.mockResolvedValueOnce({
      error: null,
    });
    ...
    expect(supabaseClient.mockAuthAdmin.deleteUser).toHaveBeenCalledWith(
      user_id,
    );
  });

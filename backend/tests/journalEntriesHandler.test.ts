...
describe('journal-entries handler', () => {
  ...
  beforeEach(() => {
    ...
    (supabaseClient.handleErrorResponse as vi.Mock).mockImplementation((res: VercelResponse, status: number, message: string) => {
  ...
  describe('POST /journal-entries', () => {
    ...
    it('should handle bulk delete request successfully', async () => {
      ...
      expect(response._getJSONData()).toEqual({ message: `${idsToDelete.length} lançamentos deletados com sucesso.` });
    ...
    it('should return 400 for invalid bulk delete IDs', async () => {
      ...
      expect(response.statusCode).toBe(400);
      expect(response._getJSONData()).toEqual({ error: 'IDs inválidos fornecidos para exclusão em massa.' });
    ...
    it('should handle bulk update status request successfully', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(response._getJSONData()).toEqual({ message: `Status de ${idsToUpdate.length} lançamentos atualizado para ${newStatus}.` });
    ...
    it('should return 400 for invalid bulk update status data', async () => {
      ...
      expect(response.statusCode).toBe(400);
      expect(response._getJSONData()).toEqual({ error: 'IDs ou status inválidos fornecidos para atualização em massa.' });
  ...
  describe('GET /journal-entries', () => {
    ...
    it('should return 403 if organization or period not found', async () => {
      ...
      expect(response._getJSONData()).toEqual({ error: 'Organização ou período contábil não encontrado para o usuário.' });
    });

    it('should handle errors from getJournalEntries service', async () => {
      (journalEntryService.getJournalEntries as vi.Mock).mockRejectedValue(new Error('Service Error'));
      ...
      expect(logger.error).toHaveBeenCalledWith(expect.any(Object), 'Journal Entries Handler: Erro inesperado na API de lançamentos:');
    });
  });
  ...
  describe('PUT /journal-entries/{id}', () => {
    ...
    it('should return 404 if journal entry not found for update', async () => {
      ...
      expect(response._getJSONData()).toEqual({ error: 'Lançamento não encontrado ou você não tem permissão para atualizar.' });
    });
  });
  ...
  describe('DELETE /journal-entries/{id}', () => {
    ...
    it('should return 404 if journal entry not found for delete', async () => {
      ...
      expect(response.statusCode).toBe(404);
      expect(response._getJSONData()).toEqual({ error: 'Lançamento não encontrado ou você não tem permissão para deletar.' });
    });
  });

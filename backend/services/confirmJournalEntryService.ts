...
export async function confirmProposedJournalEntries(
  ...
  for (const entry of proposedEntries) {
    ...
    const journalEntry = await createJournalEntry(
      {
        entry_date: new Date(entry.date).toISOString().slice(0, 10),
        description: entry.description,
        reference: entry.reference || 'AUTO-GENERATED',
      },
      organization_id,
  ...
  logger.info(
    `[ConfirmJournalEntryService] ${createdEntries.length} lançamentos criados com sucesso.`,
  )

...
export async function updateProductStockAndCost(
  ...
  try {
    ...
    if (fetchError || !product) {
      logger.error(`Product not found or fetch error for product_id: ${product_id}, user_id: ${user_id}`, fetchError);
      throw new Error(`Product not found or fetch error: ${fetchError?.message || 'Unknown error'}`);
    }
    ...
    if (transaction_type === 'purchase') {
      ...
      if (new_current_stock === 0) {
        ...
        new_unit_cost = ((product.current_stock * product.unit_cost) + (quantity * transaction_unit_cost)) / new_current_stock;
      }
    } else if (transaction_type === 'sale') {
      if (product.current_stock < quantity) {
        throw new Error(`Insufficient stock for product ${product.name}. Available: ${product.current_stock}, Requested: ${quantity}`);
      }
      new_current_stock = product.current_stock - quantity;
    ...
    if (updateError || !updatedProduct) {
      logger.error(`Error updating product stock and cost for product_id: ${product_id}, user_id: ${user_id}`, updateError);
      throw new Error(`Error updating product: ${updateError?.message || 'Unknown error'}`);
    }

    logger.info(`Product ${product.name} (ID: ${product_id}) stock updated to ${new_current_stock}, unit_cost updated to ${new_unit_cost}`);

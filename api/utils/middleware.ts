...
type ApiHandler = (req: VercelRequest, res: VercelResponse, user_id: string) => Promise<any>;
  ...
  return async (req: VercelRequest, res: VercelResponse) => {
    ...
    try {
      ...
      if (authError || !user) {
        ...
        return handleErrorResponse(res, status, `Falha na autenticação: ${authError?.message || 'Token inválido ou expirado.'}`);
      }
      ...
      const message = error instanceof Error ? error.message : 'Erro interno ao verificar autenticação.';

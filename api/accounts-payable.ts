...
export default async function (req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const parsedBody = createAccountsPayableSchema.safeParse(req.body);
      if (!parsedBody.success) {
        return handleErrorResponse(res, 400, parsedBody.error.errors.map(err => err.message).join(', '));
      }
      const newAccountPayable = { ...parsedBody.data, user_id };

      const { data, error: dbError } = await supabase
        .from('accounts_payable')
        .insert([newAccountPayable])
        .select()
        .single();
      ...
      const parsedBody = updateAccountsPayableSchema.safeParse(req.body);
      if (!parsedBody.success) {
        return handleErrorResponse(res, 400, parsedBody.error.errors.map(err => err.message).join(', '));
      }
      ...
      if (dbError) throw dbError;
      if (!data) {
        return handleErrorResponse(res, 404, 'Conta a pagar não encontrada ou não autorizada.');
      }
      return res.status(200).json(data);
      ...
      if (dbError) throw dbError;
      if (count === 0) {
        return handleErrorResponse(res, 404, 'Conta a pagar não encontrada ou não autorizada para exclusão.');
      }
    ...
    console.error('Erro inesperado na API de contas a pagar:', error);

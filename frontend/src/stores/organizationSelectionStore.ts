...
export const useOrganizationSelectionStore = defineStore('organizationSelection', () => {
  ...
  async function fetchUserOrganizations() {
    ...
    try {
      ...
      if (!targetActiveOrg) {
        targetActiveOrg =
          data.find((org) => org.is_personal && org.shared_from_user_name === null) || null
      }
  ...
  async function setActiveOrganization(organizationId: string) {
    ...
    try {
      ...
      const { organization_id, active_accounting_period_id } = await api.put<
        { organization_id: string; active_accounting_period_id: string },
        { organization_id: string }
      >( // Added active_accounting_period_id to the return type
  ...
  async function createOrganization(name: string) {
    ...
    error.value = null
    try {
      const newOrgData = await api.post<
        { organization: Organization; accounting_period: AccountingPeriod },
        { name: string }
      >('/organizations', { name })
  ...
  async function deleteOrganization(organizationId: string) {
    ...
    try {
      ...
      accessibleOrganizations.value = accessibleOrganizations.value.filter(
  ...
  async function updateOrganizationDetails(
    ...
    error.value = null
    try {
      const response = await api.put<Organization, Partial<Organization>>(
      ...
      if (index !== -1) {
        accessibleOrganizations.value[index] = {
          ...accessibleOrganizations.value[index],
          ...response,
        }
      }
      ...
      error.value =
        err instanceof Error ? err.message : 'Falha ao atualizar detalhes da organização.'
      throw err
    } finally {
      loading.value = false
    }

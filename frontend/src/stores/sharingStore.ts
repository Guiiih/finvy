...
export const useSharingStore = defineStore('sharing', () => {
  ...
  async function shareAccountingPeriod(
    ...
    try {
      ...
      const sharedPeriod = await api.post<SharedAccountingPeriod, typeof payload>('/sharing', payload);
      return sharedPeriod;
    } catch (err: unknown) {
  ...
  async function unshareAccountingPeriod(sharingId: string): Promise<void> {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Falha ao remover compartilhamento de período contábil.';
      throw err;
    } finally {
      loading.value = false;
    }

...
export const useFinancialTransactionsStore = defineStore('financialTransactions', () => {
  ...
  async function addFinancialTransaction(
  ...
  async function updateFinancialTransaction(
    type: 'payable' | 'receivable',
    id: string,
    updatedFields: Partial<Omit<FinancialTransaction, 'id' | 'created_at'>>,
  ) {
    loading.value = true
    error.value = null
    try {
      const data = await api.put<
        ...
        Partial<Omit<FinancialTransaction, 'id' | 'created_at'>> & { type: string }
      >(`/financial-transactions/${id}`, { ...updatedFields, type })
      if (type === 'payable') {

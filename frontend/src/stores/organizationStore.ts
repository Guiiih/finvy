...
export const useOrganizationStore = defineStore('organization', () => {
  ...
  const isCurrentUserOwnerOrAdmin = computed(() => {
    const currentUserRole = organizationMembers.value.find(
  ...
  async function fetchOrganizationMembers() {
    ...
    error.value = null
    try {
      const data = await api.get<UserOrganizationRole[]>(
  ...
  async function addOrganizationMember(userId: string, role: UserRoleInOrganization) {
    ...
    error.value = null
    try {
      const newMember = await api.post<
        UserOrganizationRole,
        { user_id: string; organization_id: string; role: UserRoleInOrganization }
      >(`/user-organization-roles?organization_id=${currentOrganizationId.value}`, {
        user_id: userId,
  ...
  async function updateOrganizationMemberRole(
    memberRoleId: string,
    newRole: UserRoleInOrganization,
  ) {
    if (!currentOrganizationId.value) {
      throw new Error('ID da organização não disponível.')
    }
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Falha ao atualizar papel do membro da organização.'
      throw err
    } finally {
      loading.value = false
    }

...
const methodOptions = computed(() => {
  const options: { label: string; value: CostingMethod | 'all' }[] = costingMethods.map(m => ({ label: m.label, value: m.value }));
    ...
    <div v-else>
        ...
        <div class="p-6">
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <Button @click="openSummary" icon="pi pi-chart-bar" class="px-4 py-2 text-sm font-medium border border-surface-200 rounded-md flex items-center gap-2 hover:bg-surface-50" size="small" />
              <Button @click="openMethodManagement" icon="pi pi-cog" class="px-4 py-2 text-sm font-medium border border-surface-200 rounded-md flex items-center gap-2 hover:bg-surface-50" size="small" />
            </div>
          </div>
        </div>
        <div class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div v-for="method in costingMethods" :key="method.value" class="border border-surface-200 rounded-lg p-4">
              <div class="flex items-center justify-between">
              ...
              <div class="mt-4">
                ...
                <div class="flex gap-4 mt-2 pt-2 border-t border-surface-100">
                  ...
                  <div class="w-1/2">
                    <div class="text-xs tsext-surface-500">Produtos Ativos: {{ products.length }}</div>
                    <div class="text-xs text-surface-500">{{ products.filter(p => p.is_active).length }} ativos</div>
                  </div>
                  <!-- New section for Total de Movimentações -->
                  <div class="w-1/2">
                    <div class="text-xs text-surface-500">Total de Movimentações: {{ totalMovements }}</div>
      ...
      <div class="space-y-6">
        <div class="bg-surface-200 rounded-full p-1 flex">
            <button @click="activeTab = 'movements'" :class="['flex-1 text-center py-2 px-4 rounded-full text-sm font-medium', activeTab === 'movements' ? 'bg-surface-950' : 'text-surface-500 hover:text-surface-700']">Movimentações</button>
            <button @click="activeTab = 'products'" :class="['flex-1 text-center py-2 px-4 rounded-full text-sm font-medium', activeTab === 'products' ? 'bg-surface-950' : 'text-surface-500 hover:text-surface-700']">Produtos e Métodos</button>
            <button @click="activeTab = 'alerts'" :class="['flex-1 text-center py-2 px-4 rounded-full text-sm font-medium', activeTab === 'alerts' ? 'bg-surface-950' : 'text-surface-500 hover:text-surface-700']">Alertas</button>
        </div>
        ...
        <div v-if="activeTab === 'movements'" class="space-y-6">
          ...
          <div class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1 relative">
              ...
              <input v-model="searchTerm" placeholder="Buscar por produto, SKU ou descrição..." class="w-full pl-10 pr-4 py-1 border border-surface-200 rounded-md" />
            </div>
            
            <Select v-model="selectedProduct" :options="productOptions" optionLabel="label" optionValue="value" size="small" />
            
            <Select v-model="selectedType" :options="movementTypes" optionLabel="label" optionValue="value" size="small" />

            <Select v-model="selectedMethod" :options="methodOptions" optionLabel="label" optionValue="value" size="small" />
            
            <Select v-model="selectedPeriod" :options="periods" optionLabel="label" optionValue="value" size="small" />
            
            <Button @click="openAnalysis" icon="pi pi-chart-bar" class="px-4 py-2 text-sm font-medium border border-surface-200 rounded-md flex items-center gap-2 hover:bg-surface-50" size="small" />
          </div>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full">
                <thead class="bg-surface-50">
                  <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Data</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Produto</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Método</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Tipo</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Quantidade</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Custo Unit.</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Custo Total</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Usuário</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Ações</th>
                  </tr>
                </thead>
                <tbody class="bg-primary-0">
                  <tr v-for="movement in filteredMovements.sort((a, b) => new Date(b.movement_date).getTime() - new Date(a.movement_date).getTime())" :key="movement.id" class="hover:bg-surface-50">
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="font-medium text-xs">{{ new Date(movement.movement_date).toLocaleDateString('pt-BR') }}</div>
                      <div class="text-xs text-surface-500">{{ movement.reference_id }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="font-medium text-xs">{{ getProductName(movement.product_id) }}</div>
                      <div class="text-xs text-surface-500 font-mono">{{ getProductSku(movement.product_id) }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span :class="[getCostingMethodInfo(products.find(p=>p.id === movement.product_id)?.costing_method)?.color, 'px-2 inline-flex text-sm leading-5 font-semibold rounded-full']">
                        {{ getCostingMethodInfo(products.find(p=>p.id === movement.product_id)?.costing_method)?.label.split(' ')[0] }}
                      </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span :class="[getMovementTypeColor(movement.movement_type), 'px-2 inline-flex text-sm leading-5 font-semibold rounded-full items-center gap-1']">
                        {{ movement.movement_type.charAt(0).toUpperCase() + movement.movement_type.slice(1) }}
                      </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-xs" :class="movement.movement_type === 'purchase' ? 'text-green-600' : 'text-red-600'">
                      {{ movement.movement_type === 'purchase' ? '+' : '-' }}{{ movement.quantity }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-xs font-mono">{{ formatCurrency(movement.unit_cost) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-xs font-mono">{{ formatCurrency((movement.unit_cost || 0) * movement.quantity) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-xs text-surface-500">{{ movement.created_by }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <button @click="openMovementDetail(movement)" class="p-1 text-surface-500 hover:text-surface-800">
                        <i class="pi pi-eye"></i>
                      </button>
                    </td>
                  </tr>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full">
                <thead class="bg-surface-50">
                  <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Produto</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Categoria</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Estoque</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Método Atual</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Valor PEPS</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Valor UEPS</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Valor Médio</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Valor Atual</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Ações</th>
                  </tr>
                </thead>
                <tbody class="bg-primary-0">
                  <tr v-for="product in products.filter(p => p.quantity_in_stock > 0)" :key="product.id">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span class="px-2 inline-flex text-sm leading-5 font-semibold rounded-full bg-surface-100 text-surface-800">{{ product.category }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">{{ product.quantity_in_stock }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                       <span :class="[getCostingMethodInfo(product.costing_method)?.color, 'px-2 inline-flex text-sm leading-5 font-semibold rounded-full']">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <button v-if="editingProductId !== product.id" @click="editingProductId = product.id" :disabled="loading" class="p-1 text-surface-500 hover:text-surface-800 disabled:opacity-50">
                        <i class="pi pi-pencil"></i>
                      </button>
                      <Select v-else v-model="product.costing_method" :options="costingMethods" optionLabel="label" optionValue="value" @update:modelValue="handleUpdateCostingMethod(product.id, $event)" :disabled="loading" size="small" />
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <div v-if="activeTab === 'alerts'" class="space-y-6">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div class="p-6">
                ...
                <div v-else class="space-y-3">
                  <div v-for="product in criticalProducts" :key="product.id" class="flex items-center justify-between p-3 border border-red-200 rounded-lg bg-red-50">
                    ...
                    <span class="px-2 inline-flex text-sm leading-5 font-semibold rounded-full bg-red-100 text-red-800">Estoque: {{ product.quantity_in_stock }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="border border-surface-200 rounded-lg bg-primary-0">
              ...
              <div class="p-6">
                <div v-if="lowStockProductsList.length === 0" class="text-center py-6 text-surface-500">
                  <i class="pi pi-check-circle text-5xl mx-auto mb-2 text-green-500"></i>
                ...
                <div v-else class="space-y-3">
                  <div v-for="product in lowStockProductsList" :key="product.id" class="flex items-center justify-between p-3 border border-yellow-200 rounded-lg bg-yellow-50">
                    ...
                    <span class="px-2 inline-flex text-sm leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Estoque: {{ product.quantity_in_stock }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="border border-surface-200 rounded-lg bg-primary-0">
            <div class="p-6">
              ...
              <p class="text-sm text-surface-500">Sugestões baseadas no método de custeio específico de cada produto</p>
            </div>
            <div class="p-6">
              <div class="space-y-4">
                <div v-for="product in [...criticalProducts, ...lowStockProductsList].slice(0, 5)" :key="product.id" class="flex items-center justify-between p-4 border border-surface-200 rounded-lg">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div class="text-sm text-surface-500">Estoque atual: {{ product.quantity_in_stock }} • Mínimo: {{ product.min_stock }}</div>
                    </div>
                  </div>
                  <div class="text-right">
                    ...
                    <div class="text-sm text-surface-500">Custo estimado: {{ formatCurrency(Math.max((product.max_stock || 0) - product.quantity_in_stock, (product.min_stock || 0) * 2) * (product.avg_cost || 0)) }}</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <!-- Modals -->
      <Dialog v-model:visible="showSummaryModal" modal header="Resumo dos Métodos de Custeio" :style="{ width: '50vw' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <template #header>
          <div class="flex flex-col">
            <h3 class="text-sm font-bold text-surface-900">Resumo dos Métodos de Custeio</h3>
            <p class="text-sm text-surface-500">Análise consolidada dos métodos utilizados por produto</p>
          </div>
        </template>
        <div class="p-6 space-y-6 flex-grow bg-surface-50">
          <!-- Summary Cards -->
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div v-for="method in costingMethods" :key="method.value" class="bg-primary-0 border border-surface-200 rounded-lg p-4 space-y-3">
              <div class="flex items-center gap-2">
                <i v-if="method.value === 'fifo'" class="pi pi-sort-amount-down-alt text-surface-600"></i>
                <i v-if="method.value === 'lifo'" class="pi pi-sort-amount-up-alt text-surface-600"></i>
              ...
              <div class="text-2xl font-bold text-surface-900">{{ formatCurrency(getCostingMethodSummary[method.value].value) }}</div>
              <div class="text-sm text-surface-500">{{ getTotalInventoryValue > 0 ? ((getCostingMethodSummary[method.value].value / getTotalInventoryValue) * 100).toFixed(1) : '0.0' }}% do inventário total</div>
              <div>
                <div class="w-full bg-surface-200 rounded-full h-1.5">
                  <div class="bg-surface-400 h-1.5 rounded-full" :style="{ width: products.length > 0 ? (getCostingMethodSummary[method.value].count / products.length) * 100 + '%' : '0%' }"></div>
                </div>
                <div class="flex justify-between text-sm mt-1">
                  <span class="text-surface-600">Produtos usando este método</span>
                  <span class="font-medium text-surface-800">{{ getCostingMethodSummary[method.value].count }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-primary-0 border border-surface-200 rounded-lg">
            <div class="p-4"><h4 class="font-semibold text-surface-800">Impacto da Individualização</h4></div>
            <div class="p-4 border-t grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <h5 class="text-sm text-surface-600">Se todos usassem PEPS:</h5>
                <div class="text-sm font-bold text-blue-600">{{ formatCurrency(getInventoryValueByMethod('fifo')) }}</div>
                <div class="text-sm text-surface-500">Diferença: {{ formatCurrency(getInventoryValueByMethod('fifo') - getTotalInventoryValue) }}</div>
              </div>
              <div>
                <h5 class="text-sm text-surface-600">Se todos usassem UEPS:</h5>
                <div class="text-sm font-bold text-green-600">{{ formatCurrency(getInventoryValueByMethod('lifo')) }}</div>
                <div class="text-sm text-surface-500">Diferença: {{ formatCurrency(getInventoryValueByMethod('lifo') - getTotalInventoryValue) }}</div>
              </div>
              <div>
                <h5 class="text-sm text-surface-600">Se todos usassem Médio:</h5>
                <div class="text-sm font-bold text-purple-600">{{ formatCurrency(getInventoryValueByMethod('average')) }}</div>
                <div class="text-sm text-surface-500">Diferença: {{ formatCurrency(getInventoryValueByMethod('average') - getTotalInventoryValue) }}</div>
              </div>
            </div>
          </div>
          <!-- Recommendations -->
          <div class="bg-primary-0 border border-surface-200 rounded-lg">
            <div class="p-4"><h4 class="font-semibold text-surface-800">Recomendações por Categoria</h4></div>
        ...
        <template #footer>
          <Button @click="showSummaryModal = false" label="Fechar" class="px-4 py-2 text-sm font-medium border rounded-md bg-surface-800 text-white hover:bg-surface-700" size="small" />
        </template>
      </Dialog>

      <!-- Movement Detail Modal -->
      <Dialog v-model:visible="showDetailModal" modal header="Detalhes da Movimentação" :style="{ width: '50vw' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <template #header>
          <div class="flex flex-col">
        ...
        <div v-if="selectedMovement" class="p-6 space-y-4 flex-grow">
          ...
          <div><label class="text-sm font-medium">Produto</label><div class="mt-1"><p class="text-sm font-medium">{{ getProductName(selectedMovement?.product_id) }}</p><p class="text-sm text-surface-500">{{ getProductSku(selectedMovement?.product_id) }}</p></div></div>
          <div>
        ...
        <template #footer>
          <button @click="showDetailModal = false" class="px-4 py-2 text-sm font-medium border rounded-md bg-surface-800 text-white hover:bg-surface-700">Fechar</button>
        </template>
      </Dialog>

      <!-- Method Management Modal -->
      <Dialog v-model:visible="showMethodModal" modal header="Gerenciar Métodos de Custeio" :style="{ width: '50vw' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <template #header>
          <div class="flex flex-col">
            <h3 class="text-sm font-bold text-surface-900">Gerenciar Métodos de Custeio</h3>
            <p class="text-sm text-surface-500">Configure o método de custeio para cada produto individualmente</p>
          </div>
        </template>
            ...
            <table class="min-w-full">
              ...
              <tbody class="bg-primary-0">
                <tr v-for="product in products" :key="product.id">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-surface-100 text-surface-800">{{ product.category }}</span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <Select v-model="product.costing_method" :options="costingMethods" optionLabel="label" optionValue="value" @update:modelValue="handleUpdateCostingMethod(product.id, $event)" :disabled="loading" size="small" />
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <template #footer>
          <button @click="showMethodModal = false" class="px-4 py-2 text-sm font-medium border rounded-md bg-surface-800 text-white hover:bg-surface-700">Fechar</button>
        </template>
      </Dialog>

      <!-- Analysis Modal -->
      <Dialog v-model:visible="showAnalysisModal" modal header="Análise de Movimentações por Método" :style="{ width: '50vw' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <template #header>
          <div class="flex flex-col">
            <h3 class="text-sm font-bold text-surface-900">Análise de Movimentações por Método</h3>
            <p class="text-sm text-surface-500">Estatísticas considerando métodos individualizados por produto</p>
          </div>
        </template>
        <div class="p-6 space-y-6 flex-grow bg-surface-50">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-primary-0 p-4 rounded-lg border border-surface-200">
              ...
              <p class="text-sm text-surface-500">{{ stockMovements.filter((m: StoreStockMovement) => m.movement_type === 'purchase').reduce((sum: number, m: StoreStockMovement) => sum + m.quantity, 0) }} unidades</p>
            </div>
            <div class="bg-primary-0 p-4 rounded-lg border border-surface-200">
              <h4 class="text-sm text-surface-500">Total de Saídas</h4>
              <p class="text-sm font-bold text-red-600">{{ exitMovements }}</p>
              <p class="text-sm text-surface-500">{{ stockMovements.filter((m: StoreStockMovement) => m.movement_type === 'sale').reduce((sum: number, m: StoreStockMovement) => sum + m.quantity, 0) }} unidades</p>
            </div>
            <div class="bg-primary-0 p-4 rounded-lg border border-surface-200">
              ...
              <div class="space-y-4">
                <div v-for="method in costingMethods" :key="method.value">
                  <div class="flex justify-between items-center mb-1">
                    ...
                    <span class="text-sm text-surface-600">{{ getCostingMethodSummary[method.value].count }} produtos</span>
                  </div>
                  ...
                  <div class="text-sm text-surface-500 text-right mt-1">{{ formatCurrency(getCostingMethodSummary[method.value].value) }}</div>
                </div>
              </div>
            </div>
              ...
              <div class="space-y-4">
                <div v-for="(product, index) in mostMovedProducts" :key="product.id" class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div class="flex items-center justify-center w-6 h-6 rounded-full bg-surface-200 text-surface-700 text-xs font-bold">{{ index + 1 }}</div>
                    <div>
        ...
        <template #footer>
          <button @click="showAnalysisModal = false" class="px-4 py-2 text-sm font-medium border rounded-md bg-surface-800 text-white hover:bg-surface-700">Fechar</button>
        </template>
      </Dialog>
    </div>

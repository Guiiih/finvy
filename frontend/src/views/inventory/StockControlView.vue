...
<template>
  <div class="p-4 sm:p-6 max-w-7xl mx-auto">
    ...
    <p
      v-else-if="stockControlStore.balances.length === 0"
      class="text-center p-4 bg-surface-100 border border-surface-200 rounded-lg text-surface-600 italic mt-4"
    >
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <p class="text-surface-700 mb-1">
          Quantidade: <span class="font-medium">{{ balance.quantity }}</span>
        </p>
        <p class="text-surface-700 mb-1">
          Custo Unitário Médio:
          <span class="font-medium">R$ {{ balance.unit_cost.toFixed(2) }}</span>
        </p>
        <p class="text-surface-800 text-lg font-bold mt-2">
          Valor Total: R$ {{ balance.totalValue.toFixed(2) }}
        </p>
      </div>
    </div>
  </div>

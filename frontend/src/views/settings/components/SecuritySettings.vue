  ...
  <div class="space-y-10">
    <div>
      ...
      <div class="mt-4 space-y-4 ">
        <div>
          <label for="currentPassword" class="block text-sm text-surface-600 mb-1">Senha atual</label>
          <Password
            id="currentPassword"
        ...
        <div>
          <label for="confirmNewPassword" class="block text-sm text-surface-600 mb-1">Confirmar nova senha</label>
          <Password
    ...
    <div>
        ...
        <p class="text-sm text-surface-600 mt-2 max-w-2xl">Gerencie tokens para acesso programático à sua conta</p>
    ...
    <div>
        ...
        <p class="text-sm text-surface-600 mt-2 max-w-2xl">Aplicações e serviços conectados à sua conta</p>
        <div class="mt-8 text-center text-surface-500 text-sm">
    ...
    <div>
      ...
      <p class="mt-2 text-sm text-surface-600 max-w-2xl">Excluir permanentemente sua conta e todos os dados associados. Esta ação não pode ser desfeita.</p>
      ...
      <div class="flex items-start p-2">
        <div>
            <p class="mt-2 text-sm text-surface-600">
                Esta ação é permanente e não pode ser desfeita. Todos os seus dados, organizações e configurações serão removidos permanentemente.
            </p>
        </div>
      </div>
      <template #footer>
        <Button label="Cancelar" icon="pi pi-times" @click="showDeleteModal = false" text />
        <Button label="Excluir" icon="pi pi-check" @click="handleDeleteAccount" :loading="loadingDelete" severity="danger" />
      </template>
    </Dialog>
  </div>
</template>
...
const useAuthStore = () => ({
  updatePassword: async (password: string) => { console.log('Updating password:', password); return true; },
  deleteUserAccount: async () => { console.log('Deleting account...'); return true; },
  error: null,
});
// ---
...
const handleUpdatePassword = async () => {
  ...
  if (!newPassword.value || !currentPassword.value) {
    toast.add({ severity: 'error', summary: 'Erro', detail: 'Todos os campos de senha são obrigatórios.', life: 3000 });
    return;
  }
  ...
  if (success) {
    ...
    toast.add({ severity: 'error', summary: 'Erro', detail: authStore.error || 'Falha ao atualizar a senha. Verifique sua senha atual.', life: 4000 });
  }

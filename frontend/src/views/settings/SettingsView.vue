  ...
  <div class="p-4 sm:p-6 min-h-screen">
    <div class="max-w-4xl mx-auto p-8">
      ...
      <div class="mb-8 p-6 border rounded-lg bg-surface-50">
        ...
        <div class="flex items-center space-x-6">
          <img
            ...
            alt="Avatar"
            class="h-24 w-24 rounded-full object-cover bg-surface-200 cursor-pointer shadow-lg"
      ...
      <div class="mb-8 p-6 border rounded-lg bg-surface-50">
        ...
        <div class="space-y-4">
          <div>
            <label for="fullName" class="block text-sm font-medium text-surface-700 mb-1"
              >Nome</label
            >
            <InputText
              id="fullName"
              v-model="fullName"
              class="w-full p-3 border border-surface-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
            />
          </div>
          <div>
            <label for="handle" class="block text-sm font-medium text-surface-700 mb-1"
          ...
          <div>
            ...
            <InputText
              ...
              class="w-full p-3 border border-surface-300 rounded-md bg-surface-100 cursor-not-allowed"
              disabled
            />
          </div>
          <Button
      ...
      <div class="mb-8 p-6 border rounded-lg bg-surface-50">
        ...
        <div class="space-y-4">
          <div>
            <label for="newPassword" class="block text-sm font-medium text-surface-700 mb-1"
              >Nova Senha</label
            >
            <Password
              id="newPassword"
              v-model="newPassword"
              class="rounded-md focus:ring-2 focus:ring-blue-500 transition duration-200"
              :feedback="false"
              toggleMask
            />
          </div>
          <div>
            <label for="confirmPassword" class="block text-sm font-medium text-surface-700 mb-1"
      ...
      <div class="mb-8 p-6 border rounded-lg bg-surface-50">
        ...
        <div class="space-y-4">
          ...
          <div>
            <label for="language" class="block text-sm font-medium text-surface-700 mb-1"
              >Idioma</label
            >
            <Dropdown
              id="language"
              ...
              optionLabel="name"
              optionValue="code"
              class="w-full md:w-1/4 p-button-outlined"
      ...
      <div class="mb-8 p-6 border rounded-lg bg-surface-50">
        ...
        <form @submit.prevent="handleCreateOrganization" class="space-y-4">
          <div>
            <label for="newOrganizationName" class="block text-sm font-medium text-gray-700"
          ...
          <button
            ...
            :disabled="organizationSelectionStore.loading"
          >
      ...
      <div class="mb-8 p-6 border rounded-lg bg-surface-50">
        ...
        <div
          v-if="
            organizationSelectionStore.personalOrganization &&
            organizationSelectionStore.activeOrganization?.id !==
              organizationSelectionStore.personalOrganization.id
          "
          class="mb-6 p-4 border rounded-lg bg-blue-50 border-blue-200"
        >
          <h3 class="text-xl font-semibold text-blue-800 mb-3">Seu Workspace Pessoal</h3>
          <p class="text-blue-700 mb-4">
            Você tem uma organização pessoal:
            <strong>{{ organizationSelectionStore.personalOrganization.name }}</strong
        ...
        >
          ...
          <p>
            Seu usuário não está associado a nenhuma organização ativa. Crie uma nova ou selecione
            uma existente.
          </p>
        </div>
        ...
        <div v-else>
          <h3 class="text-xl font-semibold mb-3">
            Organização Atual: {{ organizationSelectionStore.activeOrganization.name }}
            <span v-if="organizationSelectionStore.activeOrganization.is_personal">(Pessoal)</span>
          </h3>
          ...
          <div class="space-y-4 mb-6">
            ...
            <div>
              <label for="orgRazaoSocial" class="block text-sm font-medium text-gray-700"
                >Razão Social:</label
              >
              <InputText
                id="orgRazaoSocial"
            ...
            <div>
              <label for="orgMunicipio" class="block text-sm font-medium text-gray-700"
                >Município:</label
              >
              <InputText
                id="orgMunicipio"
            ...
            <Button
              label="Salvar Detalhes da Organização"
              icon="pi pi-save"
          ...
          <div class="mb-4">
            <label for="selectOrganization" class="block text-sm font-medium text-gray-700"
              >Selecionar Organização:</label
            >
            <select
            ...
            >
              <option
                v-for="org in organizationSelectionStore.accessibleOrganizations"
                :key="org.id"
                :value="org.id"
              >
                {{ org.name }} <span v-if="org.is_personal">(Pessoal)</span>
                <span v-if="org.is_shared"
                  >(Compartilhado por {{ org.shared_from_user_name }})</span
                >
              </option>
            </select>
          </div>
          ...
          <div
            v-if="
              ...
              !organizationSelectionStore.activeOrganization.is_personal
            "
            class="mb-4"
          >
            <button
          ...
          <div v-else>
            ...
            <div
              v-if="showAddMemberForm && organizationStore.isCurrentUserOwnerOrAdmin"
              class="bg-gray-50 p-6 rounded-lg shadow-inner mb-6"
            >
              ...
              <form @submit.prevent="handleSubmitMember" class="space-y-4">
                <div>
                  <label for="memberSearch" class="block text-sm font-medium text-gray-700"
                  ...
                  <div
                    ...
                    class="mt-2 border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto bg-white"
                  >
                  ...
                  <div
                    v-else-if="
                      ...
                      searchUserTerm.length > 0
                    "
                    class="mt-2 p-3 text-sm text-gray-500"
                  >
                    Nenhum usuário encontrado com este email.
                  </div>
                  <div
                    v-else-if="searchUserTerm.length > 0 && !isValidEmail"
                    class="mt-2 p-3 text-sm text-red-500"
                  >
                ...
                <div v-if="selectedUserForMembership">
                  <label class="block text-sm font-medium text-gray-700"
                    >Usuário Selecionado:</label
                  >
                  <p class="mt-1 p-3 border border-gray-300 rounded-md bg-gray-100">
                    {{ selectedUserForMembership.username || selectedUserForMembership.email }}
                  </p>
                </div>
                <div>
                  <label for="memberRole" class="block text-sm font-medium text-gray-700"
                    >Papel:</label
                  >
                  <select
                    id="memberRole"
                ...
                >
                  <div class="flex items-center space-x-4">
                    <img
                      ...
                      alt="Avatar"
                      class="h-12 w-12 rounded-full object-cover bg-surface-200 shadow-sm"
                    />
                    <div>
                      ...
                      <span
                        class="mt-1 inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-indigo-200 text-indigo-800"
                      >
                        {{ member.role }}
                      </span>
                    </div>
      ...
      <div class="p-6 border rounded-lg bg-red-50 border-red-200">
        ...
        <p class="text-red-600 mb-4">
          A exclusão da sua conta é uma ação permanente e irreversível. Todos os seus dados serão
          removidos.
        </p>
        <Button
    ...
    <Dialog
      header="Confirmar Exclusão da Organização"
      v-model:visible="showDeleteOrganizationModal"
      :modal="true"
      :style="{ width: '450px' }"
      class="p-dialog-confirm"
    >
      <div class="flex items-center p-4">
        <i class="pi pi-exclamation-triangle mr-3 text-red-500" style="font-size: 2rem"></i>
        <span class="text-surface-700">
          Você tem certeza que deseja excluir esta organização? Esta ação é irreversível e todos os
    ...
    >
      <div class="p-4">
        ...
        <div
          class="cropper-container flex justify-center items-center bg-surface-800 rounded-md overflow-hidden relative"
          style="height: 300px"
        >
          <Cropper
            ref="cropperRef"
        ...
        <div class="flex flex-col items-center mt-6 p-4 bg-surface-100 rounded-lg">
          ...
          <div class="relative mb-4">
            ...
            />
            <span
              class="absolute bottom-0 right-0 bg-blue-500 text-white rounded-full p-1 text-xs flex items-center justify-center"
        ...
        <div class="flex justify-between w-full mt-4">
          ...
          />
          <Button
            label="Salvar como foto do perfil"
            icon="pi pi-check"
...
const handleUpdatePassword = async () => {
  ...
  if (!newPassword.value) {
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: 'A nova senha não pode estar em branco.',
      life: 3000,
    })
    return
  }
  ...
  if (success) {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: authStore.error || 'Falha ao atualizar a senha.',
      life: 3000,
    })
  }
...
async function handleDeleteOrganization() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Sucesso',
      detail: 'Organização excluída com sucesso!',
      life: 3000,
    })
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: err.message || 'Falha ao excluir organização.',
      life: 3000,
    })
  }
}
...
async function handleUpdateOrganizationDetails() {
  if (!organizationSelectionStore.activeOrganization) {
    toast.add({
      severity: 'warn',
      summary: 'Atenção',
      detail: 'Nenhuma organização ativa para atualizar.',
      life: 3000,
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Sucesso',
      detail: 'Detalhes da organização atualizados com sucesso!',
      life: 3000,
    })
  } catch (err: any) {
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: err.message || 'Falha ao atualizar detalhes da organização.',
      life: 3000,
    })
  }
}
...
async function handleCreateOrganization() {
  if (!newOrganizationName.value) {
    toast.add({
      severity: 'warn',
      summary: 'Atenção',
      detail: 'O nome da organização é obrigatório.',
      life: 3000,
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: err.message || 'Falha ao criar organização.',
      life: 3000,
    })
  }
}
...
async function setActivePersonalOrganization() {
  if (organizationSelectionStore.personalOrganization) {
    try {
      await organizationSelectionStore.setActiveOrganization(
        organizationSelectionStore.personalOrganization.id,
      )
      toast.add({
        severity: 'success',
        summary: 'Sucesso',
        detail: 'Workspace pessoal ativado com sucesso!',
        life: 3000,
      })
    } catch (err: any) {
      toast.add({
        severity: 'error',
        summary: 'Erro',
        detail: err.message || 'Falha ao ativar workspace pessoal.',
        life: 3000,
      })
    }
  }
  ...
  if (selectedOrganizationId.value) {
    try {
      ...
      toast.add({
        severity: 'success',
        summary: 'Sucesso',
        detail: 'Organização ativa alterada com sucesso!',
        life: 3000,
      })
    } catch (err: any) {
      toast.add({
        severity: 'error',
        summary: 'Erro',
        detail: err.message || 'Falha ao alterar organização ativa.',
        life: 3000,
      })
    }
  }
...
async function searchUsers() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: err instanceof Error ? err.message : 'Falha ao buscar usuários.',
      life: 3000,
    })
    searchResults.value = []
    showSearchResults.value = false
...
async function handleSubmitMember() {
  if (!selectedUserForMembership.value || !newMember.value.role) {
    toast.add({
      severity: 'warn',
      summary: 'Atenção',
      detail: 'Selecione um usuário e um papel.',
      life: 3000,
    })
    return
  }
  ...
  try {
    if (editingMember.value) {
      ...
      await organizationStore.updateOrganizationMemberRole(
      ...
      toast.add({
        severity: 'success',
        summary: 'Sucesso',
        detail: 'Papel do membro atualizado com sucesso!',
        life: 3000,
      })
    } else {
      // Add new member
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: err instanceof Error ? err.message : 'Falha na operação.',
      life: 3000,
    })
  }
}
  ...
  if (confirm('Tem certeza que deseja remover este membro da organização?')) {
    try {
      ...
      toast.add({
        severity: 'error',
        summary: 'Erro',
        detail: err instanceof Error ? err.message : 'Falha ao remover membro.',
        life: 3000,
      })
    }
  }

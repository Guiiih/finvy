<template>
  <div class="p-4 sm:p-6 max-w-7xl mx-auto">
    ...
    <div class="bg-surface-50 p-6 rounded-lg shadow-md mb-6">
      <h2 class="text-xl font-semibold mb-3 text-surface-700 border-b border-surface-200 pb-2">
        {{ isEditing ? 'Editar Conta a Receber' : 'Adicionar Nova Conta a Receber' }}
      </h2>
      <form
      ...
      >
        ...
        <div class="flex items-center">
          <input
            type="checkbox"
            id="isReceived"
            v-model="newAccount.is_received"
            class="h-4 w-4 text-emerald-600 focus:ring-emerald-500 border-surface-300 rounded"
          />
        ...
        <div class="flex flex-col" v-if="newAccount.is_received">
          <label for="receivedDate" class="block text-sm font-medium text-surface-700"
    ...
    <div class="bg-surface-50 p-6 rounded-lg shadow-md mb-6">
      ...
      <div
        ...
        v-else-if="financialTransactionsStore.getUnreceivedAccounts.length > 0"
      >
        <table class="min-w-full divide-y divide-surface-200">
          ...
          <tbody class="bg-white divide-y divide-surface-200">
            <tr
              v-for="account in financialTransactionsStore.getUnreceivedAccounts"
              :key="account.id"
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                ...
                <button
                  @click="markAsReceived(account.id)"
                  v-if="!account.is_received"
                  class="px-3 py-1 bg-emerald-400 text-white rounded-md hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-opacity-50"
                >
                  Marcar como Recebida
                </button>
                <button
                  @click="handleDeleteAccountReceivable(account.id)"
      ...
      <h2 class="text-xl font-semibold mb-3 text-surface-700 border-b border-surface-200 pb-2 mt-6">
        Contas Recebidas
      </h2>

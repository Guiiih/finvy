...
const quickStats = computed<QuickStat[]>(() => [
  { title: 'Receita Bruta', value: keyMetrics.totalRevenue, change: '', trend: 'up', icon: 'pi pi-arrow-up-right' },
  { title: 'Despesas Totais', value: keyMetrics.totalExpenses, change: '', trend: 'up', icon: 'pi pi-arrow-down-left' },
  { title: 'Lucro Líquido', value: keyMetrics.netIncome, change: '', trend: 'up', icon: 'pi pi-dollar' },
  { title: 'Margem Bruta', value: keyMetrics.grossMargin, change: '', trend: 'up', icon: 'pi pi-chart-bar', isPercentage: true }
]);
...
const handleExportReport = async () => {
  if (!exportReportId.value || !reportConfig.format) {
    toast.add({ severity: 'error', summary: 'Erro', detail: 'Selecione o formato de exportação.', life: 3000 });
    return;
  }
  loading.value = true;
  try {
...
const handleSaveSchedule = async () => {
  if (!scheduleConfig.recipients[0]) {
    toast.add({ severity: 'error', summary: 'Erro', detail: 'Adicione pelo menos um destinatário', life: 3000 });
    return;
  }
  loading.value = true;
  try {
...
const handleCreateCustomReport = async () => {
  if (!customReport.name || customReport.accounts.length === 0) {
    toast.add({ severity: 'error', summary: 'Erro', detail: 'Preencha o nome e selecione pelo menos uma conta', life: 3000 });
    return;
  }
  loading.value = true;
  try {
  ...
  <main v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
      <div class="flex items-center gap-4">
        ...
        <Dropdown v-model="selectedPeriod" :options="periodOptions" optionLabel="label" optionValue="value" class="w-48" />
      </div>
      ...
      <Card v-for="(stat, index) in quickStats" :key="index">
        ...
        <template #content>
          <div class="text-2xl font-bold">
            {{ stat.isPercentage ? `${stat.value}%` : new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(stat.value) }}
          </div>
          <p class="text-xs text-surface-500 dark:text-surface-400">
            <span :class="['inline-flex items-center', stat.trend === 'up' ? 'text-green-500' : 'text-red-500']">
      ...
      <template #content>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div class="space-y-3">
            ...
            <p class="text-xs text-surface-500 dark:text-surface-400">Capacidade de pagamento de curto prazo</p>
          </div>
          <div class="space-y-3">
            <div class="flex justify-between items-center">
            ...
            <p class="text-xs text-surface-500 dark:text-surface-400">Percentual de lucro sobre receitas</p>
          </div>
          <div class="space-y-3">
            <div class="flex justify-between items-center">
            ...
            <p class="text-xs text-surface-500 dark:text-surface-400">Proporção de dívidas sobre ativos</p>
          </div>
        </div>
      </template>
    </Card>
            ...
            <template #header>
                <div class="flex items-start justify-between p-4">
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-surface-500 dark:text-surface-400 mt-1">{{ report.description }}</p>
                    </div>
            ...
            <template #content>
              <div class="space-y-4">
                ...
                <div class="flex gap-2">
                  <Button size="small" class="flex-1" @click="generateReport(report.id)" :loading="loading">
                    <i class="pi pi-eye mr-1"></i>
                    Visualizar
                  </Button>
                  <Button size="small" severity="secondary" outlined @click="handleConfigureReport(report.id)" :loading="loading">
                    <i class="pi pi-cog"></i>
                  </Button>
                  <Button size="small" severity="secondary" outlined @click="handleScheduleReport(report.id)" :loading="loading">
                    <i class="pi pi-clock"></i>
                  </Button>
                  <Button size="small" severity="secondary" outlined @click="openExportModal(report.id)" :loading="loading">
        ...
        <div class="space-y-6 p-4">
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <Dropdown v-model="reportConfig.format" :options="['pdf', 'excel', 'csv']" class="w-full" />
                </div>
                <div class="space-y-2">
                    <label>Orientação</label>
                    <Dropdown v-model="reportConfig.orientation" :options="['portrait', 'landscape']" class="w-full" />
                </div>
            </div>
            <div class="space-y-2">
                <label>Nível de Detalhamento</label>
                <Dropdown v-model="reportConfig.detailLevel" :options="['summary', 'detailed', 'full']" class="w-full" />
            </div>
            <div class="space-y-4">
                <div class="flex items-center justify-between">
            ...
            <div class="space-y-2">
                ...
                <InputText v-model="reportConfig.customTitle" placeholder="Ex: Relatório Gerencial - Agosto 2025" class="w-full" />
            </div>
            <div class="space-y-2">
                <label>Rodapé Personalizado (opcional)</label>
                <Textarea v-model="reportConfig.customFooter" placeholder="Ex: Gerado automaticamente pelo sistema FINVY" rows="2" class="w-full" />
            </div>
        </div>
        <template #footer>
        ...
        <div class="space-y-6 p-4">
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <Dropdown v-model="scheduleConfig.frequency" :options="['weekly', 'monthly', 'quarterly', 'yearly']" class="w-full" />
                </div>
                <div class="space-y-2">
                    <label>Horário</label>
            ...
            <div v-if="scheduleConfig.frequency === 'weekly'" class="space-y-2">
                ...
                <Dropdown v-model="scheduleConfig.dayOfWeek" :options="['monday', 'tuesday', 'wednesday', 'thursday', 'friday']" class="w-full" />
            </div>
            <div v-if="scheduleConfig.frequency === 'monthly'" class="space-y-2">
                <label>Dia do Mês</label>
                <Dropdown v-model="scheduleConfig.dayOfMonth" :options="Array.from({length: 28}, (_, i) => i + 1)" class="w-full" />
            </div>
            <div class="space-y-4">
                <div class="flex items-center justify-between">
            ...
            <div class="space-y-3">
                ...
                <Button label="Adicionar Destinatário" severity="secondary" outlined class="w-full" @click="addRecipient">
        ...
        <div class="space-y-6 p-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <InputText v-model="customReport.name" placeholder="Ex: Relatório de Caixa Mensal" class="w-full" />
                </div>
                <div class="space-y-2">
                    <label>Agrupar Por</label>
                    <Dropdown v-model="customReport.groupBy" :options="['account', 'category', 'date', 'month']" class="w-full" />
                </div>
            </div>
            <div class="space-y-2">
                <label>Descrição</label>
                <Textarea v-model="customReport.description" placeholder="Descrição do relatório..." rows="2" class="w-full" />
            </div>
            <div class="space-y-3">
                <label>Contas Incluídas *</label>
                <div class="border rounded-lg p-4 max-h-48 overflow-y-auto space-y-2">
                    <div v-for="account in availableAccounts" :key="account.value" class="flex items-center">
                        <Checkbox v-model="customReport.accounts" :inputId="account.value" :value="account.value" />

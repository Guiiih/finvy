<template>
  <div class="p-4 sm:p-4 md:p-6">
    ...
    <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-6" role="alert">
      ...
      <p>
        Esta é uma ferramenta de simulação. Os cálculos são estimativas e não consideram benefícios
        fiscais ou regras específicas. Consulte sempre um contador.
      </p>
    </div>
...
import type { TaxRegime, TaxRegimeHistory, TaxSimulationResult } from '@/types';
import { getErrorMessage } from '@/utils/errorUtils'

const emit = defineEmits(['tax-simulation-processed'])
...
const simulateTaxes = () => {
  ...
  if (originUF.value === destinationUF.value) {
    ...
    details.push(
      `Alíquota ICMS Interestadual Sugerida: ${aliquotaICMSInterestadual * 100}% (baseada na tabela interna de alíquotas).`,
    )
  ...
  if (hasICMSST.value && mvaPercentage.value > 0) {
    ...
    details.push(
      `Valor ICMS-ST: ${formatCurrency(valorICMSST)} (ICMS-ST = (Base ST * Alíquota Interna) - ICMS Próprio).`,
    )
  }

...
const totalOperational = computed(() => cashFlowData.details.operational.reduce((sum, item) => sum + item.value, 0));
const totalInvestment = computed(() => cashFlowData.details.investment.reduce((sum, item) => sum + item.value, 0));
const totalFinancing = computed(() => cashFlowData.details.financing.reduce((sum, item) => sum + item.value, 0));

const evolutionChartData = computed(() => ({
    labels: cashFlowData.evolution.labels,
    datasets: [
        { label: 'Entradas', data: cashFlowData.evolution.inflows, borderColor: '#22C55E', backgroundColor: 'rgba(34, 197, 94, 0.2)', fill: true, tension: 0.4 },
        { label: 'Saídas', data: cashFlowData.evolution.outflows, borderColor: '#EF4444', backgroundColor: 'rgba(239, 68, 68, 0.2)', fill: true, tension: 0.4 },
        { label: 'Fluxo Líquido', data: cashFlowData.evolution.net, borderColor: '#3B82F6', fill: false, tension: 0.4 }
    ]
}));
...
<template>
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-6">
        <Card class="text-center">
            ...
            <template #content>
                <p class="text-2xl font-bold" :class="cashFlowData.summary.operational >= 0 ? 'text-green-500' : 'text-red-500'">
        ...
        <Card class="text-center">
            ...
            <template #content>
                <p class="text-2xl font-bold" :class="cashFlowData.summary.investment >= 0 ? 'text-green-500' : 'text-red-500'">
        ...
        <Card class="text-center">
            ...
            <template #content>
                <p class="text-2xl font-bold" :class="cashFlowData.summary.financing >= 0 ? 'text-green-500' : 'text-red-500'">
        ...
        <Card class="text-center bg-surface-100 dark:bg-surface-800">
            ...
            <template #content>
                <p class="text-2xl font-bold" :class="cashFlowData.summary.netCashFlow >= 0 ? 'text-green-500' : 'text-red-500'">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card>
            ...
            <template #content>
                <div class="space-y-2">
                    <div v-for="item in cashFlowData.details.operational" :key="item.name" class="flex justify-between">
                        <span>{{ item.name }}</span>
                        <span :class="item.value >= 0 ? 'text-green-500' : 'text-red-500'">{{ formatCurrency(item.value) }}</span>
                    </div>
                    <hr class="my-2"/>
        ...
        <Card>
            ...
            <template #content>
                <div class="space-y-2">
                    <div v-for="item in cashFlowData.details.investment" :key="item.name" class="flex justify-between">
                        <span>{{ item.name }}</span>
                        <span :class="item.value >= 0 ? 'text-green-500' : 'text-red-500'">{{ formatCurrency(item.value) }}</span>
                    </div>
                    <hr class="my-2"/>
        ...
        <Card>
            ...
            <template #content>
                <div class="space-y-2">
                    <div v-for="item in cashFlowData.details.financing" :key="item.name" class="flex justify-between">
                        <span>{{ item.name }}</span>
                        <span :class="item.value >= 0 ? 'text-green-500' : 'text-red-500'">{{ formatCurrency(item.value) }}</span>
                    </div>
                    <hr class="my-2"/>

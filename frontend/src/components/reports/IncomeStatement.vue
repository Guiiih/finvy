...
<template>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <Card class="text-center">
            ...
            <template #content>
                <p class="text-3xl font-bold text-green-500">{{ formatCurrency(incomeStatementData.summary.totalRevenue) }}</p>
                <div class="h-64 mt-4">
        ...
        <Card class="text-center">
            ...
            <template #content>
                <p class="text-3xl font-bold text-red-500">{{ formatCurrency(incomeStatementData.summary.totalExpenses) }}</p>
                <div class="h-64 mt-4">
        ...
        <Card class="text-center">
            ...
            <template #content>
                <p class="text-3xl font-bold" :class="incomeStatementData.summary.netIncome >= 0 ? 'text-green-500' : 'text-red-500'">
                    {{ formatCurrency(incomeStatementData.summary.netIncome) }}
                </p>
                <p class="text-surface-500 mt-2">Margem: {{ incomeStatementData.summary.margin.toFixed(1) }}%</p>
                <div class="mt-8 space-y-3 text-left">
                    <div class="flex justify-between">
                        <span>Receita Bruta</span>
                        <span class="font-medium">{{ formatCurrency(incomeStatementData.summary.totalRevenue) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span>(-) Despesas</span>
                        <span class="font-medium text-red-500">{{ formatCurrency(incomeStatementData.summary.totalExpenses) }}</span>
                    </div>
                    <hr/>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
            ...
            <template #content>
                <div class="space-y-3">
                    <div v-for="item in incomeStatementData.revenueDetails" :key="item.name" class="flex justify-between">
                        <span>{{ item.name }} <span class="text-xs text-surface-400">{{ item.percentage }}% do total</span></span>
        ...
        <Card>
            ...
            <template #content>
                <div class="space-y-3">
                    <div v-for="item in incomeStatementData.expenseDetails" :key="item.name" class="flex justify-between">
                        <span>{{ item.name }} <span class="text-xs text-surface-400">{{ item.percentage }}% do total</span></span>
                        <span class="font-medium">{{ formatCurrency(item.value) }}</span>
                    </div>
                </div>

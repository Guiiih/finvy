...
const agingChartData = computed(() => ({
    labels: Object.keys(accountsPayableData.aging).map(k => `${k} (${accountsPayableData.aging[k as keyof typeof accountsPayableData.aging].percentage}%)`),
    datasets: [{
        data: Object.values(accountsPayableData.aging).map((v: { value: number; percentage: number }) => v.value),
...
<template>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <Card class="text-center">
            ...
            <template #content>
                <p class="text-3xl font-bold text-orange-500">{{ formatCurrency(accountsPayableData.summary.totalToPay) }}</p>
        ...
        <Card class="text-center">
            ...
            <template #content>
                <p class="text-3xl font-bold text-red-500">{{ formatCurrency(accountsPayableData.summary.overdue) }}</p>
                <p class="text-surface-500">{{ accountsPayableData.summary.overduePercentage }}% do total</p>
            </template>
        </Card>
        <Card>
            ...
            <template #content>
                <div class="space-y-4">
                    <div v-for="(data, range) in accountsPayableData.aging" :key="range">
                        ...
                        <ProgressBar :value="data.percentage" :showValue="false" class="p-progressbar-determinate" :pt="{ value: { style: { background: '#EF4444' } } }" style="height: 1.5rem"></ProgressBar>
                    </div>
                </div>
            </template>
        </Card>

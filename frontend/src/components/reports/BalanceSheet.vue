...
const totalLiabilitiesAndEquity = computed(() => 
    (balanceSheetData.detailed.liabilitiesAndEquity.current?.reduce((s, i) => s + i.value, 0) || 0) + 
    (balanceSheetData.detailed.liabilitiesAndEquity.nonCurrent?.reduce((s, i) => s + i.value, 0) || 0) + 
    (balanceSheetData.detailed.liabilitiesAndEquity.equity?.reduce((s, i) => s + i.value, 0) || 0)
);

const chartData = computed(() => ({
    labels: ['Ativo Circulante', 'Ativo Não Circulante', 'Patrimônio Líquido', 'Passivo Não Circulante', 'Passivo Circulante'],
    datasets: [
        {
            data: [
                    ...
                    <template #content>
                        <div class="flex flex-col gap-8">
                            <div class="text-center">
                                ...
                                <p class="text-3xl font-bold text-green-500">{{ formatCurrency(balanceSheetData.summary.totalAssets) }}</p>
                            </div>
                            <div class="text-center">
                                <p class="text-lg text-surface-500">Patrimônio Líquido</p>
                                <p class="text-3xl font-bold text-blue-500">{{ formatCurrency(balanceSheetData.summary.shareholdersEquity) }}</p>
                            </div>
                    ...
                    <template #content>
                        <div class="space-y-6">
                            <div>
                                ...
                                <div v-for="item in balanceSheetData.detailed.assets.current" :key="item.account" class="flex justify-between py-1 border-b border-surface-200">
                            ...
                            <div>
                                ...
                                <div v-for="item in balanceSheetData.detailed.assets.nonCurrent" :key="item.account" class="flex justify-between py-1 border-b border-surface-200">
                                    <span>{{ item.account }}</span>
                                    <span>{{ formatCurrency(item.value) }}</span>
                                </div>
                            </div>
                    ...
                    <template #content>
                        <div class="space-y-6">
                            <div>
                                ...
                                <div v-for="item in balanceSheetData.detailed.liabilitiesAndEquity.current" :key="item.account" class="flex justify-between py-1 border-b border-surface-200">
                                    <span>{{ item.account }}</span>
                                    <span>{{ formatCurrency(item.value) }}</span>
                                </div>
                            </div>
                            <div>
                                ...
                                <div v-for="item in balanceSheetData.detailed.liabilitiesAndEquity.nonCurrent" :key="item.account" class="flex justify-between py-1 border-b border-surface-200">
                                    <span>{{ item.account }}</span>
                                    <span>{{ formatCurrency(item.value) }}</span>
                                </div>
                            </div>
                            <div>
                                ...
                                <div v-for="item in balanceSheetData.detailed.liabilitiesAndEquity.equity" :key="item.account" class="flex justify-between py-1 border-b border-surface-200">
                                    <span>{{ item.account }}</span>
                                    <span>{{ formatCurrency(item.value) }}</span>
                                </div>
                            </div>

...
export async function deleteEntryLinesByJournalEntryId(
  journalEntryId: string,
  organizationId: string,
  accountingPeriodId: string,
): Promise<void> {
  await api.delete(
    `/entry-lines?journal_entry_id=${journalEntryId}&organization_id=${organizationId}&accounting_period_id=${accountingPeriodId}`,
  )
}
